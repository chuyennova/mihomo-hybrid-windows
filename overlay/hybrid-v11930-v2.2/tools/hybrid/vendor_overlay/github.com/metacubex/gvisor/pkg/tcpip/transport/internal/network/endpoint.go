...
// SetAndroidLike preserves the pre-v1.19.30 Android-like behavior where
// ordinary userspace TCP/UDP packets do not set IPv4 DF by default.
func (e *Endpoint) SetAndroidLike() {
	e.mu.Lock()
	e.pmtud = tcpip.PMTUDiscoveryDont
	e.mu.Unlock()
}
...
func (e *Endpoint) AcquireContextForWrite(opts tcpip.WriteOptions) (WriteContext, tcpip.Error) {
	...
	enforceLocalMTU := e.linuxLike && df
