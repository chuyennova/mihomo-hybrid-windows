...
func DialTCPWithBind(ctx context.Context, s *stack.Stack, localAddr, remoteAddr tcpip.FullAddress, network tcpip.NetworkProtocolNumber) (*gonet.TCPConn, error) {
	return dialTCPWithBind(ctx, s, localAddr, remoteAddr, network, false, "tcp", true)
}

// DialTCPWithBindProfile leaves SO_KEEPALIVE disabled for explicit network
// profiles so the userspace wire behavior is not forced to sing-wireguard's
// 15-second keepalive policy.
func DialTCPWithBindProfile(ctx context.Context, s *stack.Stack, localAddr, remoteAddr tcpip.FullAddress, network tcpip.NetworkProtocolNumber) (*gonet.TCPConn, error) {
	return dialTCPWithBind(ctx, s, localAddr, remoteAddr, network, false, "tcp", false)
}

// dialTCPWithBind creates a TCP endpoint with explicit address-family and
// IPv6-only semantics.
func dialTCPWithBind(ctx context.Context, s *stack.Stack, localAddr, remoteAddr tcpip.FullAddress, network tcpip.NetworkProtocolNumber, v6Only bool, networkName string, forceKeepalive bool) (_ *gonet.TCPConn, retErr error) {
